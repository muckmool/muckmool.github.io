...
class CWidget(QWidget): 
    ...
    def __init__(self):
        ...
        self.pushButton = QPushButton('전체지우기')  
        self.pushButton.clicked.connect(self.totalErase)            
        grid.addWidget(self.pushButton, 1, 0)

        self.pushButton = QPushButton('Undo')  
        self.pushButton.clicked.connect(self.unDo)            
        grid.addWidget(self.pushButton, 2, 0)
        ...
        self.lineResult = QLineEdit()
        grid.addWidget(self.lineResult, 2, 0)
    ...
    def saveFile(self):
        file_name = self.lineEdit.text() + '.png'
        #img = QPixmap(self.view.grab(self.view.sceneRect().toRect()))
        rec = QRect(0, 0, 1103, 487)
        img = QPixmap(self.view.grab(rec))
        img.save(file_name)
        now = time
        self.lineResult.setText(now.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def unDo(self):
        max = len(self.view.scene.items())
        if max > 0:
            self.view.scene.removeItem(self.view.scene.items()[0])
        else:
            pass
